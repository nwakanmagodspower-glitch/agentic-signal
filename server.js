/**
 * GODSPOWER AGENTIC SIGNAL - MASTER SERVER (FINAL)
 * ------------------------------------------------
 * FEATURES:
 * 1. User Memory (Session Restore)
 * 2. Auto-Approve Telegram Requests
 * 3. Personalized Welcome Message
 * 4. Smart Expiry (Weighted Random)
 * 5. 3-Tier Pricing Logic
 * 6. Signal History & Admin Broadcast
 */
    ...
    socket.on('check_user_status', (userId) => {
        if (websiteUsers[userId] && websiteUsers[userId].tier > 0) {
            socket.emit('account_unlocked', { 
                ...
                message: "WELCOME BACK! RESTORING VIP ACCESS 🚀" 
            });
        }
    });
});
...
function generateSignal() {
    ...
    return { pair, price: (1.0000 + Math.random()).toFixed(4), rsi, decision, expiry, tierRequired, timestamp: new Date().toLocaleTimeString() };
}
